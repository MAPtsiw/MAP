...
let inscritosNoEvento = [] //Vai guardar os utilizadores que estão inscritos num evento que vai decorrer dentro de dois dias

let eventosNotificar = []
...
//Vai servir para mandar mail apenas uma vez por dia
let mandarMail = true
...
window.onload = function () {
    ...
    let confirmarDiaHoje = datinha.getDate()
    let diaEmlocalStorage = 0

    this.localStorage.setItem('confirmarDiaHoje', JSON.stringify(confirmarDiaHoje))
    if (this.localStorage.getItem('confirmarDiaHoje')) {
        diaEmlocalStorage = JSON.parse(this.localStorage.getItem('confirmarDiaHoje'))
    }

    if (diaEmlocalStorage != confirmarDiaHoje && mandarMail == false) mandarMail = true
    ...
    if (this.localStorage.getItem('recomendados')) {
        ...
        for (let i = 0; i < a.length; i++) {
            let b = new Recomendado(a[i]._eventoId, a[i]._userId[0])
            if (a[i]._userId.length > 1) {
                for (let k = 0; k < a[i]._userId.length; k++) {
                    b.userId = a[i]._userId[k]
                }
            }
    ...
    if (localStorage.getItem('indexUtilizador')) indexUtilizador = JSON.parse(localStorage.getItem('indexUtilizador'))

    //Saber como está a variavel mandarMail
    if (localStorage.getItem('mandarMail')) mandarMail = JSON.parse(this.localStorage.getItem('mandarMail'))
    ...
    if (logged == true) {
        console.log(utilizadores[indexUtilizador].tipoUtilizador)
    ...
    if (logged == true) {
        ...
        if (mandarMail) gestaoAlertas()

        //Gerir as notificações dos eventos na navbar
        eventosDentroDe2Dias()
        escreverParaANavabar()

        document.getElementById('notificacoes').addEventListener('click', function () {
            preencherModalNotificacao(eventosNotificar, 'containerEventos2Dias')
        })
    ...
    if (document.getElementById('containerCardsCarrosel') != null) {
        ...
        for (let i = eventos.length - 1; i >= eventos.length - 7; i--) {
    ...
    formRegistar.addEventListener('submit', function (e) {
        ...
        if (tipo == 'Tipo Utilizador') {
            continuar = false
            alert('Tem que selecionar um tipo de utilizador válido')
        }
        ...
        if (password == confPass && continuar == true) { //Verificar se as pass's são iguais
            //Criar Utilizador
            let novoUtilizador = new Utilizador(nome, password, eMail, foto, tipo, "", "", "", 0) //Porque   me está a por o tipo de utilizador na foto????
    ...
    formLogin.addEventListener('submit', function (e) {
        ...
        if (ok == true) {
            ...
            localStorage.setItem('UtilizadorMostrar', JSON.stringify(utilizadores[indexUtilizador]))
    ...
    btnLogoff.addEventListener('click', function () {
        ...
        let paginaAtual = window.location.href.split('/')
        paginaAtual = paginaAtual[paginaAtual.length - 1]
        if (paginaAtual == 'projeto2_Perfil.html' || paginaAtual == 'projeto2_Admin.html') window.location.replace('projeto2.html')
    ...
    if (calendario != null) {
        calendario.addEventListener('click', function (event) {
            ...
            if (conf != "" && conf == true) {
                ...
                if (temEventos == true) {
                    document.getElementById('ohPaEle').focus()
                    document.getElementById('ohPaEle').innerHTML = ""
        ...
        btnPesquisar.addEventListener('click', function () {
            ...
            if (searchData != 'Todos') {
                if (filtraditos.length > 0) {
                    filtraditos = filtrarPorData(filtraditos, searchData);
                }
                else {
    ...
    if (document.getElementById('paginaEvento') != null) {
        if (localStorage.getItem('eventoMostrar')) { //FAzer tudo aqui dentro? Por agora sim, sim porque se não vai estar a comentar o quê....
            ...
            console.log(eventoLindo)

            let leData = ""
            if (eventoLindo.data[0] != undefined) leData = eventoLindo.data[0].split(';')[0]
            ...
            if (eventoLindo.imagem == "") this.document.getElementById('eventimg').setAttribute('src', 'evento1.png')
            else this.document.getElementById('eventimg').setAttribute('src', eventoLindo.imagem)
            ...
            if (eventoLindo.pontuacao != undefined) {
                ...
                this.document.getElementById('pontuacaoMedia').innerHTML = eventoLindo.pontuacao.toFixed(2) + " /5";
            }
            ...
            let maltaInscrita = []
            console.log(maltaInscrita)
            for (let i = 0; i < eventoLindo.inscritos.length; i++) {
                for (let j = 0; j < utilizadores.length; j++) {
                    if (eventoLindo.inscritos[i] == utilizadores[j].id) maltaInscrita.push(utilizadores[j])
                }
            }
                        ...
                        if (!seguir) {
                            ...
                            for (let i = 0; i < recomendados[indexi].userId.length; i++) {
                                if (recomendados[indexi].userId[i] == utilizadores[indexUtilizador].id) {
                                    seguir2 = false
                                    mensagem = 'Já recomendas te este evento'
                                }
                            }
                        }
                        else {
    ...
    let botaoTestemunhar = document.getElementById('btnTestemunhar')
    if (botaoTestemunhar != null) {
        ...
        botaoTestemunhar.addEventListener('click', function () {
            ...
            if (utilizadores[indexUtilizador].tipoUtilizador == 'Estudante') {
                ...
                alert('Só os estudantes podem testemunhar')
            }
        })
    ...
    if (document.getElementById('paginaPerfil') != null) {
        if (this.localStorage.getItem('UtilizadorMostrar')) {
            ...
            if (this.localStorage.getItem('editarPerfil')) {
                editar = JSON.parse(this.localStorage.getItem('editarPerfil'))
            ...
            for (let i = 0; i < eventos.length; i++) {
                for (let k = 0; k < eventos[i].inscritos.length; k++) {
                    if (eventos[i].inscritos[k] == loUtilizador._id) {
                        ...
                        eventosRecomendados2.push(eventos[i])
            ...
            if (contadori == 0) document.getElementById("eventosParticipados").innerHTML = "Ainda não participou em nenhum evento"
                ...
                for (let i = 0; i < eventosRecomendados2.length; i++) {
                    if (logged == true) mostrar = paNaoSei(eventosRecomendados2[i].userId)

                    if (eventosRecomendados2[i].data[0] != undefined) {
                        preencherCatalogo(eventosRecomendados2[i].nome,
                            eventosRecomendados2[i].imagem,
                            eventosRecomendados2[i].descricao,
                            eventosRecomendados2[i].data[0].split(';')[0],
                            eventosRecomendados2[i].data[0].split(';')[1],
                            eventosRecomendados2[i].pontuacao
                            , i, mostrar, 'eventosParticipados')
                    }
                    else if (eventosRecomendados2[i].data[0] == undefined) {
                        preencherCatalogo(eventosRecomendados2[i].nome,
                            eventosRecomendados2[i].imagem,
                            eventosRecomendados2[i].descricao,
                            "Por Anunciar",
                            "Por Anunciar",
                            eventosRecomendados2[i].pontuacao //Fazer alguma coisa quando a pontuação não estiver definida
                            , i, mostrar, 'eventosParticipados')
                    }
                }
            }
            ...
            for (let i = 0; i < eventos.length; i++) {
                for (let k = 0; k < arrayRecomendados.length; k++) {
                    ...
                    if (eventos[i].id == arrayRecomendados[k]) {
                        ...
                        eventosRecomendados3.push(eventos[i])
            ...
            console.log(eventosRecomendados3)
            if (contadorii == 0) document.getElementById("eventosRecomendados").innerHTML = "Ainda não recomendou em nenhum evento"
            else {
                for (let i = 0; i < eventosRecomendados3.length; i++) {
                    if (logged == true) mostrar = paNaoSei(eventosRecomendados3[i].userId)

                    if (eventosRecomendados3[i].data[0] != undefined) {
                        preencherCatalogo(eventosRecomendados3[i].nome,
                            eventosRecomendados3[i].imagem,
                            eventosRecomendados3[i].descricao,
                            eventosRecomendados3[i].data[0].split(';')[0],
                            eventosRecomendados3[i].data[0].split(';')[1],
                            eventosRecomendados3[i].pontuacao
                            , i, mostrar, 'eventosRecomendados')
                    }
                    else if (eventosRecomendados3[i].data[0] == undefined) {
                        preencherCatalogo(eventosRecomendados3[i].nome,
                            eventosRecomendados3[i].imagem,
                            eventosRecomendados3[i].descricao,
                            "Por Anunciar",
                            "Por Anunciar",
                            eventosRecomendados3[i].pontuacao //Fazer alguma coisa quando a pontuação não estiver definida
                            , i, mostrar, 'eventosRecomendados')
                    }
                }
            }

            //Preencher aqui os recomendados
            if (editar == true && (eventosRecomendados3.length > 0 || eventosRecomendados2.length > 0)) eventsToRecomend()
...
function preencherCarrosel(cardName, cardImage, cardDescricao, cardData, cardHora, cardPontuacao, indice) {
    ...
    if (indice == eventos.length - 1) div1.setAttribute('class', 'carousel-item col-sm-4 active');
    ...
    if (indice == eventos.length - 1) divImg.setAttribute('class', 'card-img-top h img-fixed');
    else divImg.setAttribute('class', 'card-img-top h img-fluid')

    console.log(cardImage)
    if (cardImage == "") divImg.setAttribute('src', "images/evento3.jpg")
    else divImg.setAttribute('src', cardImage)
    ...
    let pontuar = 0
    //console.log(cardPontuacao)

    if (typeof cardPontuacao == 'number') pontuar = cardPontuacao.toFixed(2)
    else pontuar = 'Por definir'
    ...
    detalhes.innerHTML = `Data: ${cardData} ||  
                        Hora: ${cardHora}
                        <br>
                        Pontuação: ${pontuar}` //Fica melhor com 3,5 / 5
    ...
    btnDetalhes.setAttribute('class', 'btn btn-primary ' + eventos[indice].id + ' btn-saberMais')
...
function preencherCatalogo(cardName, cardImage, cardDescricao, cardData, cardHora, cardPontuacao, indice, mostrarMenu/* = false*/, idDoContainer) { //A parte de filtrar é feita antes, esta função só preenche
    ...
    let idEvento = eventos.filter(function (eve) {
        return eve.nome == cardName
    })

    console.log(idEvento[0].id)
    ...
    btn1.setAttribute('class', 'dropdown-item ' + idEvento[0].id)
    ...
    btn2.setAttribute('class', 'dropdown-item ' + idEvento[0].id)
    ...
    cartao.setAttribute('class', 'card col-lg-4 ' + idEvento[0].id) //Em vez de lg, secalhar fica md
    ...
    if (cardPontuacao != undefined) pont.textContent = "Pontuação: " + cardPontuacao.toFixed(2)
    else pont.textContent = 'Pontuação: Por definir'
    ...
    botaoVer.setAttribute('class', 'btn btn-orange ' + idEvento[0].id + ' btn-saberMais')
            ...
            if (contadorDias <= fimMes) {
                ...
                if (k < indici && i == 0) dias[i].children[k].innerHTML = `<i style="color:grey">${contadorDiasQueFaltam}<i>`
                    ...
                    if (contadorDias == diaDeHoje && mes == esteMes) {
                        dias[i].children[k].style.backgroundColor = "rgb(91, 104, 216, 0.5)"
                        dias[i].children[k].style.color = "green"
                    }
                    else {
...
function melhorPontuados() {
    ...
    return array3 //Array com os 5 eventos mais bem classificados, nah, se depois quisermos só os cinco melhores pegamos nos cinco primeiros do array devolvido
...
function editarEvento(e) {
    ...
    if (caLinda.categoria.length == 1) document.getElementById('CategoriaEventoAlt').value = caLinda.categoria[0] //Isto vai ter que passar para um ciclo for.....
        ...
        document.getElementById('CategoriaEventoAlt').value = categoriasTodas
    }
    //Imagem
...
function verEvento(e) {
    ...
    console.log(atacari)
    console.log('atatatatataatatatatatatatat')
    ...
    localStorage.setItem("eventoMostrar", JSON.stringify(pimba[0]))
}
...
function realmentePontuar() { //Em principio as matemáticas vão ser feitas na classe
    ...
    document.getElementById('botaoPontuar').style.display = 'none'
}
...
function cabeconas(arrayPassar) { //Este array deve ser o array que é devolvido na função inscritosEvento()
    ...
    for (let i = 0; i < 4; i++) { //arrayPassar.length, não vai ser este o tamanho, porque vão aparecer no máximo 3 caras + 1 (+) 
        ...
        if (arrayPassar[i] != undefined) {
            leImagem.setAttribute('class', arrayPassar[i].id + ' pessoas-inscritas')
            div.setAttribute('data-toggle', 'tooltip')
            div.setAttribute('data-placement', 'top')
            div.setAttribute('title', arrayPassar[i].nome)

            //data-toggle="tooltip" data-placement="top" title="Hooray!"
            leImagem.addEventListener('click', guardarUtilizador)
        } else {
        ...
        else if (arrayPassar[i] == undefined) imagemLinda = 'images/favicon.png'
        else if (arrayPassar[i].fotografia == "") imagemLinda = "images/6.jpg"
        else if (arrayPassar[i].fotografia != "") {
            imagemLinda = arrayPassar[i].fotografia
...
function guardarUtilizador(e) {
    ...
    let idAtacar = e.target.className.split(' ')[0]

    let tag = e.target.toString().split(' ')[1]
    console.log(tag)
    ...
    if (tag == 'HTMLImageElement]') window.location.replace('projeto2_Perfil.html')
}
...
//Enviar mails usando o Node.js

//Ir buscar a libraria
// "use strict";
// const nodemailer = require('nodemailer');

// // Aceder ao mail pelo qual se vão enviar os mails
// var transporter = nodemailer.createTransport({
//     service: 'gmail',
//     auth: {
//         user: 'mapmepmip@gmail.com',
//         pass: 'javardos69'
//     }
// });

//Função para ir buscar os eventos que estão a dois dias de acontecer e saber quem está inscrito nestes

function gestaoAlertas() {

    //Dizer a data e a hora, se disponivel, ao mandar o mail
    let mesDoEvento = 0
    let diaDoEvento = 0
    let horaDoEvento = ""
    ...
    for (let i = 0; i < eventos.length; i++) {
        if (eventos[i].data[0] != undefined) {
            mesDoEvento = parseInt(eventos[i].data[0].split(';')[0].split('-')[1])
            diaDoEvento = parseInt(eventos[i].data[0].split(';')[0].split('-')[2])
            if (eventos[i].data[0].split(';')[1] != "") horaDoEvento = parseInt(eventos[i].data[0].split(';')[1])
            /*let mailis = */auxiliarGestaoAlertas(diaDoEvento, mesDoEvento, i)
            // mandarMails(mailis, i) //Assim sei que evento está a ser 'visto'
        }
    }

    //Dizer que já não se mandam mais mails
    mandarMail = false
    localStorage.setItem('mandarMail', JSON.stringify(mandarMail))
}

function auxiliarGestaoAlertas(dia, mes, indice) {
    ...
    let mailsAlertar = []
    let continuar = false
    //FAzer outro if, para confirmar se os dois dias que faltam para o evento acontecer não estão em meses diferentes, pouco provavel mas aconatece
    if (dia - 2 == diaHoje && mes == mesHoje) {
        if (eventos[indice].inscritos.length > 0) continuar = true
    }

    if (continuar) {
        for (let i = 0; i < eventos[indice].inscritos.length; i++) {
            for (let k = 0; k < utilizadores.length; k++) {
                if (eventos[indice].inscritos[i] == utilizadores[k].id) {
                    mailsAlertar.push(utilizadores[k].mail)
                    // inscritosNoEvento.push(utilizadores[k].id)
                }
            }
        }
    }

    // console.log(inscritosNoEvento)

    // return mailsAlertar
}

//Função para mandar os mails
function mandarMails(loArray, indice) {

    let mailsInscritos = ""
    if (loArray.length == 1) mailsInscritos = loArray[0]
    else if (loArray.length > 1) {
        mailsInscritos = loArray[0]
        for (let i = 1; i < loArray.length; i++) {
            mailsInscritos += ', ' + loArray[i]
        }
    }

    let conteudoEmail = `O evento ${eventos[indice].nome} decorre dentro de 2 dias, não precisas de agradecer`

    var mailOptions = {
        from: 'mapmepmip@gmail.com',
        to: mailsInscritos,
...
//Saber quais são os eventos que vão decorrer dentro de 2 oumenos dias e em que o utilizador logado estaja enscrito
function eventosDentroDe2Dias() { //Tenho que confirmar se alguém está logado ou não

    let idAtacare = utilizadores[indexUtilizador].id

    for (let i = 0; i < eventos.length; i++) {
        if (eventos[i].inscritos.length > 0) {
            utilizadoresInscritos(i, idAtacare)
        }
    }
}

//Procurar os utilizadores do evento
function utilizadoresInscritos(indice, idAtacar) {


    let diaHoje = dataAtual.getDate()
    let mesHoje = dataAtual.getMonth() + 1;

    let diaEvento = eventos[indice].data[0].split(';')[0].split('-')[2]
    let mesEvento = eventos[indice].data[0].split(';')[0].split('-')[1]

    if (diaEvento >= diaHoje && mesHoje == mesEvento) {
        console.log(diaEvento, diaHoje)
        if (diaEvento - diaHoje <= 2 && mesEvento == mesHoje) {
            console.log('ata')
            for (let j = 0; j < eventos[indice].inscritos.length; j++) {
                if (eventos[indice].inscritos[j] == idAtacar) {
                    eventosNotificar.push(eventos[indice])
                }
            }
        }
    }
    console.log(eventosNotificar)
}
//Função para escrever mensagem para a navBar
function escreverParaANavabar() { //Provavelmente vai ter que mudar de nome

    let sininho = document.getElementById('notificacoes')
    if (sininho != null) {
        if (eventosNotificar.length > 0) {
            sininho.style.display = 'inline-block'

            sininho.innerHTML = `<i class="fas fa-bell iconNav"></i>(${eventosNotificar.length})`

        }
        else {
            sininho.style.display = 'none'
        }
    }
}

//Função para preencher o modal de eventos a notificar
function preencherModalNotificacao(array, idContainer) {

    let listaEventos = document.getElementById(idContainer)
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        //Item da lista
        let li = document.createElement('li')
        li.setAttribute('class', 'listaEventos')

        listaEventos.appendChild(li)

        //Conteudo do item da lista

        //Linha que vai conter o conteudo
        let divRow = document.createElement('div')
        divRow.setAttribute('class', 'row rowEbentos')

        li.appendChild(divRow)

        let divImagem = document.createElement('div')
        divImagem.setAttribute('class', 'col-4 divDaImagem')

        divRow.appendChild(divImagem)

        let img = document.createElement('img')
        img.setAttribute('class', 'imagemEvento')
        let imgSrc = ""
        if (array[i].imagem == "") imgSrc = 'images/evento2.jpg'
        else imgSrc = array[i].imagem
        img.setAttribute('src', imgSrc)

        divImagem.appendChild(img)

        let divBody = document.createElement('div')
        divBody.setAttribute('class', 'col-8')

        divRow.appendChild(divBody)

        let divNome = document.createElement('div')
        divNome.setAttribute('class', 'nomeDele')
        divNome.textContent = array[i].nome

        divBody.appendChild(divNome)

        let a = document.createElement('a')
        a.setAttribute('class', `btn saber-mais ${array[i].id} btnModal`)
        a.textContent = 'Saber Mais'
        a.setAttribute('href', 'projeto2_Evento.html')
        a.addEventListener('click', verEvento)


        divBody.appendChild(a)

        let divTempoFalta = document.createElement('div')
        divTempoFalta.setAttribute('class', 'tempoQueFalta')
        //Tempo que falta
        let diaDeHoje = dataAtual.getDate()
        let diaDoEvento = parseInt(array[i].data[0].split(';')[0].split('-')[2])

        if (diaDoEvento - diaDeHoje == 0) divTempoFalta.textContent = 'Hoje!'
        else divTempoFalta.textContent = `Daqui a ${diaDoEvento - diaDeHoje} dias`

        divBody.appendChild(divTempoFalta)
    }
}
//#######################################################################################################################
//#######################################################################################################################
